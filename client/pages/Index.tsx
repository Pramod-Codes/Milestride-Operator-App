...
function Vehicle({ setView }: { setView: (v: View) => void }) {
  return <><TopBar title="Vehicle details" onBack={() => setView("home")} /><main className="page"><div className="vehicle-title"><div><p className="eyebrow">VEHICLE</p><h2>MS 2048</h2></div><StatusBadge>Active</StatusBadge></div><div className="hero-bike"><img src={bikeImage} /><div className="image-label"><MapPin size={13} /> University Campus</div></div><div className="vehicle-specs"><div><span>Model</span><strong>E Bike X1</strong></div><div><span>Battery</span><strong className="battery-value"><BatteryCharging size={15} />86%</strong><div className="progress"><i style={{ width: "86%" }} /></div></div><div><span>Range</span><strong>32 km</strong></div><div><span>Status</span><strong className="green-text">In service</strong></div><div><span>Last updated</span><strong>{dateLabel}, 08:30 AM</strong></div></div><div className="sticky-actions"><button className="secondary" onClick={() => setView("issue")}><History size={17} /> View history</button><button className="primary" onClick={() => setView("inspection")}><ClipboardCheck size={17} /> Inspect now</button></div></main></>;
}
