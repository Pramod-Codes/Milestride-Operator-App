...
function ThemeToggle() {
  ...
  return <button className="theme-switch" aria-label="Toggle dark mode" aria-pressed={dark} onClick={() => setDark(value => !value)}><span className="switch-track"><span className="switch-knob">{dark ? <Sun size={11} /> : <Moon size={11} />}</span></span><span>{dark ? "Dark mode on" : "Dark mode off"}</span></button>;
}
